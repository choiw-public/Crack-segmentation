...
def scale_up_if_too_small(image, min_length_limit):
    ...
    if image_shape[-1] == 3:
        ...
        return tf.image.resize_images(image, new_shape, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR, align_corners=True)
...
def image_preprocess_for_segmentation(img, gt, config):
    img = tf.cast(img, config.dtype)
    if config.is_train:
        if gt is None:
            raise ValueError('gt should not be none in training')
        if config.is_train:
            crop_height = config.input_size[0]
            crop_width = config.input_size[1]
            ...
            if config.min_length_limit:
                ...
                img = scale_up_if_too_small(img, min_length_limit)
                gt = scale_up_if_too_small(gt, min_length_limit) if gt is not None else None
            if config.max_length_limit:
                max_length_limit = float(max(max(crop_height, crop_width), config.max_length_limit))
                img = scale_down_if_too_large(img, max_length_limit)
                gt = scale_down_if_too_large(gt, max_length_limit) if gt is not None else None
            # Data augmentation by randomly scaling the inputs.
            scale = get_random_scale(min_scale_factor, max_scale_factor)
            if min_scale_factor != 1.0 or max_scale_factor != 1.0:
                if min_scale_factor > max_scale_factor:
            ...
            img, gt = random_crop([img, gt], crop_height, crop_width)
            img.set_shape([crop_height, crop_width, None])
            gt.set_shape([crop_height, crop_width, 1]) if gt is not None else None
            ...
            if config.warp_prob > 0.0:
                ...
                img, gt = tf.py_func(warp, [img, gt, config.warp_prob, config.warp_ratio, config.warp_crop_prob], [tf.uint8, tf.uint8])
